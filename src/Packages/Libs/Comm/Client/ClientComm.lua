...
function ClientComm:BuildObject(inboundMiddleware: Types.ClientMiddleware?, outboundMiddleware: Types.ClientMiddleware?)
	local obj = {}
	local rfFolder = self._instancesFolder:FindFirstChild("Owl_RF")
	local reFolder = self._instancesFolder:FindFirstChild("Owl_RE")
	local rpFolder = self._instancesFolder:FindFirstChild("Owl_RP")
	if rfFolder then
