--[[
			Trove
			by Stephen Leitnick and recoded by Dev_Abrahel
--]]
...
local GenericObjectCleanupMethods = { "Destroy", "Disconnect", "destroy", "disconnect" }

--

local RunService = game:GetService("RunService")

-- > // Func : Get Object Cleanup Function \\ < --
...
local function GetObjectCleanupFunction(object, cleanupMethod)
	...
	elseif t == "RBXScriptConnection" then
		return "Disconnect"
	elseif t == "table" then
		for _, genericCleanupMethod in GenericObjectCleanupMethods do
...
--[=[
	...
	Internally, tracked objects are stored as keys in a hashmap
	(`object -> cleanupMethod`) rather than in an array that must be
	linearly scanned. This makes `Add`, `Remove`, and promise cleanup
	O(1) instead of O(n), and avoids allocating a wrapper table per
	tracked object.

	One consequence: adding the *exact same object reference* twice
	no longer creates two independent tracked entries. The second
	`Add` call simply overwrites the cleanup method for that key, so
	the object will only be cleaned up once, not twice. This is
	almost never something real code depends on; if you genuinely
	need two independent cleanup handles for one object, wrap one of
	them (e.g. `trove:Add(function() object:Destroy() end)`).
]=]
local Trove = {}
Trove.__index = Trove
...
function Trove:AddPromise(promise)
	...
	if promise:getStatus() == "Started" then
		promise:finally(function()
			...
			-- Promise already settled on its own; just drop the tracking
			-- entry, don't re-invoke :cancel() on an already-finished promise.
			self._objects[promise] = nil
		end)
...
function Trove:Add(object: any, cleanupMethod: string?): any
	...
	if object == nil then
		error("Cannot add nil to trove", 2)
	end
	local cleanup = GetObjectCleanupFunction(object, cleanupMethod)
	self._objects[object] = cleanup
	return object
end
...
function Trove:Remove(object: any): boolean
	...
	self:_cleanupObject(object, cleanupMethod)
	return true
end
...
function Trove:Clean()
	...
	self._cleaning = true
	-- Swap out the live table first: any cleanup function that itself
	-- calls trove:Destroy()/trove:Clean() re-entrantly (e.g. via a
	-- Destroying connection) sees _cleaning already true and just
	-- returns, instead of racing over the same table we're iterating.
	local objects = self._objects
	self._objects = {}
	for object, cleanupMethod in objects do
		self:_cleanupObject(object, cleanupMethod)
	end
	self._cleaning = false
end
